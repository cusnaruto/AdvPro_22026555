...
void Game::snakeMoveTo(Position pos) {
	//  START CODE HERE
	CellType positionNow = getCellType(pos);
		if (positionNow == CELL_SNAKE || positionNow == CELL_OFF_BOARD)
		{
			status = GAME_OVER;
			return;
		}
	else if (positionNow == CELL_CHERRY)
	{
		score++;
		snake.eatCherry();
		addCherry;
...
void Game::addCherry()
{
    do {
        ...
        Position randomPos (rand()%getWidth(), rand()%getHeight());
